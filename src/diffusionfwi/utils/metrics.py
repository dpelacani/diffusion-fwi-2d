...
def _make_tissue_mask(vp_ms):
    """
    Morphological tissue and skull masks for single 128x128 velocity map (m/s).

    Steps:
        1. raw velocity thresholding
        2. binary closing for bridging gaps in skull ring
        3. identify brain interior as largest connected fill component
        4. apply binary erosion to obtain margin to skull boundary

    Returns:   
        tissue_mask: bool array [H, W] with valid tissue interior pixels
        skull_mask: bool array [H, W] with raw skull pixels (vp >= 1650)
        valid: bool (false if tissue mask empty / mask failed)
    """
    skull_mask = vp_ms >= SKULL_THRESH_MS
 
    skull_ring = binary_closing(skull_mask, structure=disk(CLOSING_RADIUS))
    filled_int = binary_fill_holes(skull_ring)
    tissue = filled_int & ~skull_mask
 
    labeled, n_comp = label(tissue)
    if n_comp == 0:  # no valid tissue mask found
        return np.zeros_like(skull_mask), skull_mask, False
    elif n_comp > 1: # keep only largest connected component (interior tissue)
        sizes = np.bincount(labeled.ravel())
        sizes[0] = 0
        tissue_clean = labeled == sizes.argmax()
    else:            # single connected group representing brain interior
        tissue_clean = tissue.astype(bool)
 
    tissue_mask = binary_erosion(tissue_clean, structure=disk(EROSION_RADIUS))
    return tissue_mask.astype(bool), skull_mask.astype(bool), bool(tissue_mask.any())
...
def tissue_statistics(real, gen):
    ...
    def _collect(vp_ms_batch):
        """ Run morphological masking over a batch to get tissue and skull pixels. """
        tissue_vals, skull_vals = [], []
        n_excluded = 0 # track number of failed maskings

        for i in range(vp_ms_batch.shape[0]):
            vp_ms = vp_ms_batch[i, 0].cpu().numpy()   # [H, W]
            tissue_mask, skull_mask, valid = _make_tissue_mask(vp_ms)
 
            if not valid:
                n_excluded += 1
                continue
 
            tissue_vals.append(vp_ms[tissue_mask])
            skull_vals.append(vp_ms[skull_mask])
        
        n_total = vp_ms_batch.shape[0]
        ...
        tissue_all = np.concatenate(tissue_vals)
        skull_all  = np.concatenate(skull_vals)
        return (
            float(tissue_all.mean()), float(tissue_all.std()),
            float(skull_all.mean()),  float(skull_all.std()),
            n_valid, n_excluded,
        )

    (t_mean_real, t_std_real, sk_mean_real, sk_std_real, n_valid_real, n_excl_real) = _collect(real_ms)
    (t_mean_gen, t_std_gen, sk_mean_gen, sk_std_gen, n_valid_gen, n_excl_gen) = _collect(gen_ms)

    return {
        "soft_tissue_mean_real": t_mean_real,
        "soft_tissue_std_real":  t_std_real,
        "soft_tissue_mean_gen":  t_mean_gen,
        "soft_tissue_std_gen":   t_std_gen,
        "skull_mean_real":       sk_mean_real,
        "skull_std_real":        sk_std_real,
        "skull_mean_gen":        sk_mean_gen,
        "skull_std_gen":         sk_std_gen,
        "tissue_n_valid_real":   n_valid_real,
        "tissue_n_excl_real":    n_excl_real,
        "tissue_n_valid_gen":   n_valid_gen,
        "tissue_n_excl_gen":    n_excl_gen,
    }
...
def compute_lpips(samples, device, num_pairs=300):
    """
