...
class DenoisingGuidanceOperator(Operator):
    """
    Diffusion denoising guidance (DG/DG-C) operator for FWI.
    Implements the GuidanceOperator protocol (see base_operator.py) as a
    process_model pipeline step.
    ...
    def __init__(
        ...
        super().__init__(name="diffusion_denoising_guidance")
        ...
        logger.info("(diffusionfwi) Diffusion Denoising Guidance FWI Pipeline initialized (split=%s) with device=%s}", split, device)
    ...
    def forward(self, vp, **kwargs):
        ...
        if self.visual_dir:
            ...
            np.save(f"{self.visual_dir}/iter{self.iteration:03d}_vp.npy",
                    vp.data.copy())
        ...
        logger.info(
            "Diffusion denoising guidance at iter %d: t=%d alpha_skull=%.3f alpha_tissue=%.3f",
        ...
        zero_tissue = (self.warmstart_steps is not None and self.warmstart_steps > 0
                       and self.guidance_step <= self.warmstart_steps)
