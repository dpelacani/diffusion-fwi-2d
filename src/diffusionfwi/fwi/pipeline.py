...
from diffusionfwi.dataset.buildDataset import AcousticNormalization, ReverseAcousticNormalization
from diffusionfwi.diffusion import DiffusionProcess
from diffusionfwi.utils import merge_channels, split_channels
from diffusionfwi.constants import NORM_MIN, NORM_MAX
...
class DiffusionFWIPipeline:
    ...
    def run(
        ...
        if self.split:
            volume_split = split_channels(volume) # shape [1, 2, x_dim, x_dim]
            zero_tissue = update_kwargs.get("zero_tissue", False)

            if zero_tissue:
                volume_input = torch.cat([
                    torch.zeros_like(volume),   # ch0: no tissue signal
                    volume.clone(),             # ch1: full normalised FWI
                ], dim=1)
            else:
                volume_input = volume_split.clone()

            volume_fwi = volume_input.clone() # save FWI state for blending

            if visual_dir and visual_iter is not None:
                os.makedirs(visual_dir, exist_ok=True)
                torch.save(volume_input[:, 0:1].contiguous().cpu(),
                        f"{visual_dir}/iter{visual_iter:03d}_fwi_ch0.pt")
                torch.save(volume_input[:, 1:2].contiguous().cpu(),
                        f"{visual_dir}/iter{visual_iter:03d}_fwi_ch1.pt")

            # Move to device for diffusion processing
            volume = volume_input.to(self.device)
            volume = self._denoise(volume, t_start, random_seed, verbose, visual_dir, visual_iter)

            if visual_dir and visual_iter is not None:
                torch.save(volume[:, 0:1].cpu(), f"{visual_dir}/iter{visual_iter:03d}_diffusion_ch0.pt")
                torch.save(volume[:, 1:2].cpu(), f"{visual_dir}/iter{visual_iter:03d}_diffusion_ch1.pt")
            ...
            if visual_dir and visual_iter is not None:
                torch.save(ch0.cpu(), f"{visual_dir}/iter{visual_iter:03d}_blended_ch0.pt")
                torch.save(ch1.cpu(), f"{visual_dir}/iter{visual_iter:03d}_blended_ch1.pt")
        ...
        if visual_dir and visual_iter is not None:
            torch.save(volume.cpu(), f"{visual_dir}/iter{visual_iter:03d}_merged_norm.pt")
    ...
    def _denoise(
        ...
        if visual_dir and visual_iter is not None: 
            os.makedirs(visual_dir, exist_ok=True)
            # Save the exact noise added (epsilon)
            torch.save(noise.cpu(), f"{visual_dir}/iter{visual_iter:03d}_noise_added.pt")
            # Save the noisy image (consistent 3-digit naming)
            torch.save(noisy_vol.cpu(), f"{visual_dir}/iter{visual_iter:03d}_noisy_img.pt")
            # Save the t value used
            with open(f"{visual_dir}/iter{visual_iter:03d}_t_val.txt", "w") as f:
                f.write(str(int(t_val.item())))
        ...
        if visual_dir and visual_iter is not None:
            torch.save(volume.cpu(), f"{visual_dir}/iter{visual_iter:03d}_samples.pt")
    ...
    def _sample_from_start_time(
        ...
        for idx in tqdm(reversed(range(num_steps)), desc="Diffusion Sampling", total=num_steps, leave=False):
            ...
            if visual_dir and visual_iter is not None and idx == num_steps - 1:
                os.makedirs(visual_dir, exist_ok=True)
                avg_e_pred = e_pred.mean(dim=0, keepdim=True)          # [1, C, H, W]
                avg_noisy  = noisy_vol.mean(dim=0, keepdim=True)       # [1, C, H, W]
                torch.save(avg_e_pred.cpu(),
                           f"{visual_dir}/iter{visual_iter:03d}_noise_predicted.pt")
                # Score direction: noisy_img - predicted_noise
                torch.save((avg_noisy - avg_e_pred).cpu(),
                           f"{visual_dir}/iter{visual_iter:03d}_score_direction.pt")
