...
def plot_vp_and_wavelet(model="vp_996782", max_iter=72):
    ...
    fig, axs = plt.subplots(
        2, 2, figsize=(12, 8), gridspec_kw={"height_ratios": [2, 1]}
    )
    axs[0, 1].axis("off")
    # Create a meshgrid for the x and y coordinates
    x = (
        np.arange(problem.space.shape[1]) * problem.space.spacing[1] * 1e3
    )  # Convert to mm
    y = (
        np.arange(problem.space.shape[0]) * problem.space.spacing[0] * 1e3
    )  # Convert to mm
    ...
    transducer_label = axs[0, 0].scatter(
        x_coords, y_coords, color="orange", s=10, label="Receivers"
    )
    ...
    for idx, id in enumerate(shot_ids):
        ...
        if idx == 0:
            source_label = axs[0, 0].scatter(
                source_x, source_y, color="red", s=50, marker="*", label="Sources"
            )
        else:
    ...
    axs[0, 0].set_title(
        f"Velocity Model (Vp) f_center = 0.25 MHz \n max_freqs = {max_freqs} MHz"
    )
    # Optional grid
    # axs[0, 0].grid(alpha=0.3)
    axs[0, 0].legend(
        handles=[source_label, transducer_label],
        loc="lower left",
    ...
    # wavelet
    time_axis = (
        np.arange(source_wavelets.size) * 0.08e-6 * 1e6
...
def plot_vp_gif_wavelet(data_dir, max_freqs=None, max_iter=96):
    ...
    source_scatter = axs[0, 0].scatter(
        source_x, source_y, color="red", s=50, marker="*", label="Sources"
    )
    ...
    ani = animation.FuncAnimation(
        fig, update, frames=vp_array.shape[0], interval=300, blit=False
    )
