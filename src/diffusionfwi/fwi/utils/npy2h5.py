...
def npy2h5(
    path=None, is_data=False, data=None, name="BrainModel.h5", save_path="./", **kwargs
):
    """
    Create a true model for the Brain Ultrasound dataset and save it as an HDF5 file.

    Args:
        path: Path to the .npy file containing the model data.
        is_data: If True, use the provided model_data instead of loading from model_path.
        data: Numpy array containing the model data if is_data is True.
    ...
    if is_data:
        ...
        assert (
            path is not None
        ), "Path to .npy file must be provided if is_data is False."
        model_data = np.load(path)
    shape = model_data.shape  # (320, 256)

    # define space parameters
    extra = kwargs.get("extra", (50, 50))
    absorbing = kwargs.get("absorbing", (40, 40))
    spacing = kwargs.get("spacing", (0.5e-3, 0.5e-3))
    extended_shape = np.array(
        [shape[0] + extra[0] * 2, shape[1] + extra[1] * 2], dtype="int64"
    )
    ...
    time_info = {
        "num": kwargs.get("t_num", 2500),
        "start": kwargs.get("t_start", 0.0),
        "step": kwargs.get("t_step", 8e-8),
        "stop": kwargs.get("t_num", 2500) * kwargs.get("t_step", 8e-8),
    }
    ...
    with h5py.File(f"{save_path}/{name}", "w") as f:
