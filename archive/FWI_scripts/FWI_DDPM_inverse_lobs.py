...
class DDPMVpUpdate:
    ...
    def update_vp(self, vp, iteration):
        ...
        # update the number of diffusion steps based on t
        self.steps = t

        # schedule alpha based on the current iteration, alpha controls the strength of the diffusion update
        # this weight is gradually decreased over the iterations to allow for finer updates as we approach convergence
        alpha = self.schedule_alpha(iteration, self.max_iter)
        ...
        self.diffusion_state = self.diffusion_sample(x_t, t, self.steps)
        # postprocess the denoised velocity model to the original format
        updated_vp = postprocess_vp(self.diffusion_state)

        # blend the updated vp with the original vp using alpha
        new_vp = vp.alike()
        new_vp.data[:] = (1 - alpha) * vp.data + alpha * updated_vp.cpu().numpy()
        return new_vp
    ...
    def diffusion_sample(self, x, t_start, num_steps):
        ...
        with torch.no_grad():
            ...
            for t in reversed(range(start_t, t_start)):
                ...
                # denoise x
                x = self.diffusion.reverse_diffusion(
                    x, t_tensor, self.model(x, t_tensor), z
                )
    ...
    def schedule_alpha(self, iteration, max_iter):
        alpha_start = 0.3
        alpha_end = 0.05
        alpha = alpha_start - (alpha_start - alpha_end) * (iteration / max_iter)
        alpha = max(alpha_end, min(alpha_start, alpha))
        return alpha
...
async def main(runtime):
    ...
    # the vp can be manipulated by accessing the Numpy array `vp.data`
    # vp.data[:] = np.zeros_like(vp.data)
    # vp.data[100:200, :] = 1400.
    ...
    diffusion_model_path = os.path.join(
        absolute_path, "diffusion model/model_added_res.pt"
    )
    ...
    for i, (block, freq) in enumerate(optimisation_loop.blocks(num_blocks, max_freqs)):
        ...
        for iteration in block.iterations(num_iters):
            ...
            # use DDPM to denoise the vp model every iteration
            use_diffusion = True
