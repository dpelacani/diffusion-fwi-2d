...
def compute_ms_ssim(
    file_dir1, file_dir2, data_range=1.0, suffix=None, end_frame="00096"
):
    """
    ...
    """
    if suffix:
        image1 = get_vp_from_file(
            file_dir1, file_suffix=suffix, true_model=True, end_frame=end_frame
        )
    else:
    ...
    if image1.shape != image2.shape:
        raise ValueError(
            "Input images must have the same shape for MS-SSIM computation."
        )
...
def plot_comparison(raw_dir, true_dir, diff_dir, suffix=None, end_frame="00096"):
    ...
    if suffix:
        true_vp = get_vp_from_file(
            true_dir, file_suffix=suffix, true_model=True, end_frame=end_frame
        )
    else:
...
def plot_diff(true_dir, inverse_dir, raw_dir, suffix=None, end_frame="00096"):
    ...
    im1 = ax1.imshow(
        raw_diff_vp.squeeze().cpu().numpy(), cmap="terrain", origin="lower"
    )
...
def plot_loss(
    all_loss_diff=None, all_loss_fwi=None, raw_fwi=False, freqs=None, iters=None
):
    """
    ...
    plt.axvline(
        x=iters[0], color="red", linestyle="--", label=f"max_freq = {freqs[0]} MHz"
    )
    plt.axvline(
        x=iters[1], color="green", linestyle="--", label=f"max_freq = {freqs[1]} MHz"
    )
    plt.axvline(
        x=iters[2], color="orange", linestyle="--", label=f"max_freq = {freqs[2]} MHz"
    )
    plt.axvline(
        x=iters[3], color="purple", linestyle="--", label=f"max_freq = {freqs[3]} MHz"
    )
...
def plot_all_gifs(file_dir, start_frame=1, end_frame=96, file_suffix=".h5"):
    ...
    """
    grad_array = load_grad_sequence(
        f"{file_dir}/anastasio2D-VpGrad-", start_frame, end_frame, file_suffix
    )
    processed_grad_array = load_grad_sequence(
        f"{file_dir}/anastasio2D-ProcessedVpGrad-", start_frame, end_frame, file_suffix
    )
    vp_array = load_vp_sequence(
        f"{file_dir}/anastasio2D-Vp-", start_frame, end_frame, file_suffix
    )
